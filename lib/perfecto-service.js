...
class PerfectoService {
    before () {
        ...
        browser.addCommand('setValueImmediate', function (locator, value) {
            let elemId = $(locator).value.ELEMENT
            browser.setImmediateValue(elemId, value)
        })

        browser.addCommand('waitUntil', function (condition, timeout = WAITFOR_TIMEOUT, timeoutMsg, interval = 1000) {
        ...
        // browser.addCommand('isExisting', function () {
         ...
         * Report commands
         */
        browser.addCommand('perfReportVerify', function (assertFnc) {
            try {
                // browser.debug();
                assertFnc()
                browser.perfReportAssert(assertFnc.error_message, true)
                return true
            } catch (err) {
         ...
         * Device Utils
         */
        browser.addCommand('perfInstallApp', function (filePath, shouldInstrument) {
            let params = {
                file: filePath
            }
            if (shouldInstrument) {
        ...
        browser.addCommand('prefStartApp', function (app, by) {
            browser.execute('mobile:application:open', this.getAppParams(app, by))
        })
        // by = 'name' or 'identifier'
        browser.addCommand('perfCloseApp', function (app, by, ignoreExceptions = false) {
            try {
                browser.execute('mobile:application:close', this.getAppParams(app, by))
            } catch (err) {
                if (!ignoreExceptions) {
                    throw err
                }
        ...
        browser.addCommand('perfGetApplicationInfo', function (property) {
            let params = {
                property: property
            }
        ...
        browser.addCommand('perfVerifyAppInfo', function (propertyName, propertyValue) {
            let assertMethod = () => assert.equal(propertyValue, browser.perfGetAppInfo(propertyName), `${propertyName} should be ${propertyValue}`)
            return browser.verify(assertMethod)
        })
        browser.addCommand('perfAssertAppInfo', function (propertyName, propertyValue) {
            assert.equal(propertyValue, browser.perfGetAppInfo(propertyName), `${propertyName} must be ${propertyValue}`)
        })
        ...
        browser.addCommand('perfWaitForPresentImageVisual', function (img, seconds) {
            assert.equal(browser.perfFindImage(img, seconds), 'true', `Image: ${img} should be visible after ${seconds} seconds`)
        })
        browser.addCommand('perfFindImage', function (img, timeout) {
            let context = browser.getContext()
            browser.switchToContext('VISUAL')
            let params = {
                content: img,
            ...
            browser.switchToContext(context)
            return result.toString()
        })
        browser.addCommand('perfAssertVisualImage', function (img, seconds = 180) {
            return assert.equal(browser.perfFindImage(img, seconds), true, `Image: ${img} must be visible after ${seconds} seconds`)
        })
        browser.addCommand('perfVerifyVisualImage', function (img) {
            let assertMethod = () => assert.equal(browser.perfFindImage(img, 180), true, `Image: ${img} should be visible`)
            return browser.verify(assertMethod)
        })
        /**
        ...
        browser.addCommand('perfAssertVisualText', function (text) {
            return assert.equal(browser.perfFindText(text, 180), true, `Text: ${text} should be present`)
        })
        browser.addCommand('perfVerifyVisualText', function (text) {
            let assertMethod = () => assert.equal(browser.perfFindText(text, 180), true, `Text: ${text} should be visible`)
            return browser.verify(assertMethod)
        })
        ...
        browser.addCommand('perfAssertTimezone', function (timezone) {
            ...
            browser.deviceTimezone = deviceTimezone
         ...
         *            of the webview
         */
        browser.addCommand('perfTouchObject', function (locator, addressBar) {
            ...
            let rect = browser.getElementRect(browser.getElement(locator))
         ...
         *            object to slide
         */
        browser.addCommand('perfSlideObjectLeft', function (locator) {
            ...
            browser.slideObject(locator, startX, endX, y)
        })
         ...
         *            - y point to end on
         */
        browser.addCommand('perfSlideObject', function (locator, xStartMult, xEndMult, yStartMult, yEndMult = yStartMult) {
        ...
        browser.addCommand('perfGetOffset', function (addressBar, context = 'NATIVE_APP') {
            ...
            browser.switchToContext(curContext) // Switch back to the original context
    ...
    beforeScenario (scenario) {
        ...
        console.log(' browser.scenarioTags' + browser.scenarioTags)
