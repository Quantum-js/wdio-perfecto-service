...
function getAppParams (by, app) {
    let params = {}
    params[by] = app
    return params
}

function customWaitUntil(condition, timeout , timeoutMsg, interval) {
...
function parseFailureJsonFile(actualMessage) {

    let failureConfigLoc =  browser.options.perfectoOpts.failureReasonConfig || 'src/main/resources/failureReasons.json'
    if (fs.exists(failureConfigLoc) != true) {

        console.log('Ignoring Failure Reasons because JSON file was not found in path: ', failureConfigLoc);
        return null;
    }

    let messages=require(failureConfigLoc)

    for (let message in messages) {
        if (message.StackTraceErrors == null) {
            console.log('Failure Reason JSON file has wrong formmat, please read here https://developers.perfectomobile.com/pages/viewpage.action?pageId=31103917: ', failureConfigLoc);
            return null;

        }

        for (let error in message.StackTraceErrors)	{
...
class PerfectoService {
    before () {
        ...
        browser._waitUntil = browser.waitUntil;
        browser.waitUntil = function (condition, timeout, timeoutMsg, interval) {
            ...
            customWaitUntil(condition, timeout, timeoutMsg, interval);
            browser.timeouts({
                "implicit": global.IMPLICIT_TIMEOUT
            });
        }
        ...
        browser.addCommand('perfStartApp', function (by, app) {
            browser.execute('mobile:application:open', getAppParams(by, app))
        })
        // by = 'name' or 'identifier'
        browser.addCommand('perfCloseApp', function (by, app, ignoreExceptions = false) {
            try {
        ...
        // by = 'name' or 'identifier'
        browser.addCommand('perfCleanApp', function (by, app) {
            browser.execute('mobile:application:clean', getAppParams(by, app))
        })
        // by = 'name' or 'identifier'
        browser.addCommand('perfUninstallApp', function (by, app) {
        ...
        browser.addCommand('perfWaitForPresentTextVisual', function (text, timeout) {
            assert.equal(browser.perfFindText(text, timeout), 'true', `Text: ${text} should be present after ${seconds} seconds`)
        })
        browser.addCommand('perfWaitForPresentImageVisual', function (img, timeout, threshold = 90, needleBound = 25) {
            assert.equal(browser.perfFindImage(img, timeout, threshold, needleBound), 'true', `Image: ${img} should be visible after ${seconds} seconds`)
        })
        ...
        browser.addCommand('perfAssertVisualImage', function (img, timeout, threshold = 90, needleBound = 25) {
            let message = `Image: ${img} must be visible after ${timeout} seconds`
            let assertMethod = () => assert.equal(browser.perfFindImage(img, timeout, threshold, needleBound), 'true', message)
            return browser.perfAssert(assertMethod, message)
        })
        browser.addCommand('perfVerifyVisualImage', function (img, timeout, threshold = 90, needleBound = 25) {
            let message = `Image: ${img} must be visible after ${timeout} seconds`
            let assertMethod = () => assert.equal(browser.perfFindImage(img, timeout, threshold, needleBound), 'true', message)
            return browser.perfAssert(assertMethod, message)
        })
        ...
        browser.addCommand('perfRotateDevice', function (by, restValue) {
            let params = {}
            params[by] = restValue
        ...
        browser.addCommand('perfSetLocation', function (by, location) {
            let params = {}
            params[by] = location
        ...
        // by = 'name' or 'identifier'
        browser.addCommand('perfStartImageInjection', function (repositoryFile, by, app) {
            let params = {}
        ...
        /**
         * Gets the current application scale for the device
         *
        ...
        browser.addCommand('perfSetPickerWheel', function (locator, direction, value, offset = 0.15) {

            String.prototype.replaceAll = function(search, replacement) {
                let target = this
                return target.replace(new RegExp(search, 'g'), replacement)
            }
            const search = '[^\\x00-\\x7F]'
            value = value.replaceAll(search, '')
            //_.replace(value,new RegExp("az","g"),"[^\\x00-\\x7F]")
            //value =  value.replace("[^\\x00-\\x7F]", "");

            for(let name = this.perfPickerwheelGet(locator).replaceAll(search, ''); name != value; name = browser.perfPickerwheelGet(locator).replaceAll(search, '')) {
                console.log(name)
                browser.perfPickerwheelStep(locator, direction, offset)
            }

        })

        browser.addCommand('perfPickerwheelGet', function (locator) {
            return $(locator).getAttribute('value')
        })

        browser.addCommand('perfPickerwheelStep', function (locator, direction, offset = 0.15) {
            ...
            browser.execute('mobile: selectPickerWheelValue', params)
        })
    }
