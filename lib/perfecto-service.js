...
class PerfectoService {
    before () {
        ...
        browser.addCommand('perfAssertLocation', function (location) {
            ...
            return assert.equal(deviceLocation, location, `Device Location ${deviceLocation.toString()} must be equal ${location}`)
        })

        browser.addCommand('perfVerifyLocation', function (location) {
            let deviceLocation = this.perfGetDeviceLocation()
            let assertMethod = () => assert.equal(deviceLocation, location, `Device Location ${deviceLocation.toString()} should be equal ${location}`)
        ...
        browser.addCommand('perfAssertTimezone', function (timezone) {
            ...
            return assert.equal(deviceTimezone, timezone, `Device timezone ${deviceTimezone} must be equal ${timezone}`)
        })

        browser.addCommand('perfVerifyTimezone', function (timezone) {
            let deviceTimezone = browser.perfGetTimezone();
            let message = `Device Location ${deviceTimezone} should be equal to ${timezone}`;
            let assertMethod = () => assert.equal(deviceTimezone, timezone, message);

            return browser.perfReportVerify(assertMethod, message);
        })
